...
public class CalculatorUI {
        ...
        comboCalculatorType.addItemListener(event -> {
            ...
            switch (selectedItem) {
                case "Standard":
                    ...
                    break;
                case "Scientific":
                    window.setSize(WINDOW_WIDTH + 80*2 + MARGIN_X, WINDOW_HEIGHT);
    ...
    private void initButtons(int[] columns, int[] rows) {
        ...
        btnCos = createButton("cos", columns[5], rows[1]);
        ...
        btne = createButton("e", columns[5], rows[2]);
        btne.addActionListener(event ->{
            if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()))
                return;

            if (go) {
                typedValue = Math.exp(Double.parseDouble(inputScreen.getText()));
                if (Pattern.matches("[-]?[\\d]+[.][0]*", String.valueOf(typedValue))) {
                    inputScreen.setText(String.valueOf((int) typedValue));
                } else {
                    inputScreen.setText(String.valueOf(typedValue));
        ...
        btnDeg = createButton("°", columns[5], rows[3]);
        btnDeg.addActionListener(event ->{
            if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()))
                return;

            if (go) {
                typedValue = Math.toDegrees(Double.parseDouble(inputScreen.getText()));
                if (Pattern.matches("[-]?[\\d]+[.][0]*", String.valueOf(typedValue))) {
                    inputScreen.setText(String.valueOf((int) typedValue));
                } else {
                    inputScreen.setText(String.valueOf(typedValue));
        ...
        btnPi = createButton("π", columns[5], rows[4]);
        btnPi.addActionListener(event ->{
            if (addToDisplay) {
                if (Pattern.matches("[0]*", inputScreen.getText())) {
                    inputScreen.setText(String.valueOf(Math.PI));
                } else {
                    inputScreen.setText(inputScreen.getText() + String.valueOf(Math.PI));
                }
            } else {
                inputScreen.setText(String.valueOf(Math.PI));
                addToDisplay = true;
    ...
    private void applyTheme(Theme theme) {
        ...
        btnSin.setForeground(hex2Color(theme.getTextColor()));
        btnCos.setForeground(hex2Color(theme.getTextColor()));
        btne.setForeground(hex2Color(theme.getTextColor()));
        btnDeg.setForeground(hex2Color(theme.getTextColor()));
        ...
        btnSin.setBackground(hex2Color(theme.getOperatorBackground()));
        btnCos.setBackground(hex2Color(theme.getOperatorBackground()));
        btne.setBackground(hex2Color(theme.getOperatorBackground()));
        btnDeg.setBackground(hex2Color(theme.getOperatorBackground()));
